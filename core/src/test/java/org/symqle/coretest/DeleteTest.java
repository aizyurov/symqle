...
public class DeleteTest extends SqlTestCase {
    ...
    public void testDeleteAll() throws Exception {
        ...
        final String sql = deleteStatementBase.showUpdate(new GenericDialect());
        assertSimilar("DELETE FROM person", sql);
        final String sql2 = deleteStatementBase.showUpdate(new GenericDialect());
        assertSimilar(sql, sql2);
    }

    public void testAdapt() throws Exception {
        final AbstractDeleteStatementBase deleteStatementBase = person.delete();
        final String sql1 = deleteStatementBase.showUpdate(new GenericDialect());
        final String sql2 = AbstractDeleteStatement.adapt(deleteStatementBase).showUpdate(new GenericDialect());
        assertEquals(sql1, sql2);
        final String sql3 = AbstractDeleteStatementBase.adapt(deleteStatementBase).showUpdate(new GenericDialect());
    ...
    public void testWhere() throws Exception {
        ...
        final String sql = deleteStatement.showUpdate(new GenericDialect());
        assertSimilar("DELETE FROM person WHERE person.id = ?", sql);
        assertSimilar(sql, deleteStatement.showUpdate(new GenericDialect()));
    }

    public void testSubqueryInWhere() throws Exception {
        final Person child = new Person();
        final String sql = person.delete().where(child.id.where(child.parentId.eq(person.id)).exists()).showUpdate(new GenericDialect());
    ...
    public void testWrongCondition() throws Exception {
        final Person child = new Person();
        try {
            final String sql = person.delete().where(child.name.eq("John")).showUpdate(new GenericDialect());
