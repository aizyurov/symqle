...
public class FactorTest extends SqlTestCase {
    ...
    public void testWhere() throws Exception {
        final String sql = person.id.opposite().where(person.smart.asBoolean()).showQuery(new GenericDialect());
    ...
    public void testBooleanValue() throws Exception {
        String sql = person.id.where(person.id.opposite().asBoolean()).showQuery(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(- T0.id)", sql);
    }
