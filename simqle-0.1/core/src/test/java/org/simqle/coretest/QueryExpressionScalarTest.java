...
public class QueryExpressionScalarTest extends SqlTestCase {
    ...
    public void testShow() throws Exception {
        final String sql = employee.id.union(manager.id).show();
        assertSimilar("SELECT T1.id AS C0 FROM employee AS T1 UNION SELECT T2.id AS C0 FROM manager AS T2", sql);
        final String sql2 = employee.id.union(manager.id).show(GenericDialect.get());
    ...
    public void testOrderAsc() throws Exception {
        final String sql = employee.id.union(manager.id).orderAsc().show();
        assertSimilar("SELECT T1.id AS C0 FROM employee AS T1 UNION SELECT T2.id AS C0 FROM manager AS T2 ORDER BY C0", sql);
    }

    public void testOrderDesc() throws Exception {
        final String sql = employee.id.union(manager.id).orderDesc().show();
