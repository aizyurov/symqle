...
public class TermTest extends SqlTestCase {
    ...
    public void testWhenClause() throws Exception {
        final String sql = person.name.isNotNull().then(person.id.mult(2)).show();
    ...
    public void testLike() throws Exception {
        final String sql = person.id.where(person.id.mult(1).like(DynamicParameter.create(Mappers.STRING, "12%"))).show();
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE T0.id * ? LIKE ?", sql);
    }

    public void testNotLike() throws Exception {
        final String sql = person.id.where(person.id.mult(1).notLike(DynamicParameter.create(Mappers.STRING, "12%"))).show();
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE T0.id * ? NOT LIKE ?", sql);
    }

    public void  testLikeString() throws Exception {
        final String sql = person.id.where(person.id.mult(1).like("12%")).show();
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE T0.id * ? LIKE ?", sql);
    }
    
    public void  testNotLikeString() throws Exception {
        final String sql = person.id.where(person.id.mult(1).notLike("12%")).show();
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE T0.id * ? NOT LIKE ?", sql);
    }
    ...
    public void testList() throws Exception {
        new Scenario() {
            @Override
            protected void runQuery(final DataSource datasource, final AbstractTerm<Number> term) throws SQLException {
                final List<Number> list = term.list(datasource);
        ...
        new Scenario() {
            @Override
            protected void runQuery(final DataSource datasource, final AbstractTerm<Number> term) throws SQLException {
                final List<Number> list = term.list(new DialectDataSource(GenericDialect.get(), datasource));
                assertEquals(1, list.size());
                assertEquals(123L, list.get(0).longValue());
            }
        }.play();
    ...
    public void testScroll() throws Exception {
        new Scenario() {
            @Override
            protected void runQuery(final DataSource datasource, final AbstractTerm<Number> term) throws SQLException {
                term.scroll(datasource, new Callback<Number>() {
                    int callCount = 0;
        ...
        new Scenario() {
            @Override
            protected void runQuery(final DataSource datasource, final AbstractTerm<Number> term) throws SQLException {
                term.scroll(new DialectDataSource(GenericDialect.get(), datasource), new Callback<Number>() {
                    int callCount = 0;
    ...
    private static abstract class Scenario {
        public void play() throws Exception {
            ...
            final String queryString = term.show();
            ...
            runQuery(datasource, term);
            verify(datasource, connection, statement, resultSet);
        }

        protected abstract void runQuery(final DataSource datasource, final AbstractTerm<Number> term) throws SQLException;
    }
