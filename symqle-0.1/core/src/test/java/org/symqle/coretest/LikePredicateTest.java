...
public class LikePredicateTest extends SqlTestCase {
    ...
    public void testIsNull() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').isNull()).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) IS NULL", sql);
    }

    public void testIsNotNull() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').isNotNull()).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) IS NOT NULL", sql);
    }

    public void testEq() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').eq(person.alive)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) = T0.alive", sql);
    }

    public void testNe() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').ne(person.alive)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) <> T0.alive", sql);
    }

    public void testGt() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').gt(person.alive)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) > T0.alive", sql);
    }

    public void testGe() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').ge(person.alive)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) >= T0.alive", sql);
    }

    public void testLt() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').lt(person.alive)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) < T0.alive", sql);
    }

    public void testLe() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').le(person.alive)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) <= T0.alive", sql);
    }

    public void testEqValue() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').eq(true)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) = ?", sql);
    }

    public void testNeValue() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').ne(true)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) <> ?", sql);
    }

    public void testGtValue() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').gt(true)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) > ?", sql);
    }

    public void testGeValue() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').ge(true)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) >= ?", sql);
    }

    public void testLtValue() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').lt(true)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) < ?", sql);
    }

    public void testLeValue() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').le(true)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) <= ?", sql);
    }

    public void testIn() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').in(child.alive.where(child.parentId.eq(person.id)))).show(new GenericDialect());
    ...
    public void testNotIn() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').notIn(child.alive.where(child.parentId.eq(person.id)))).show(new GenericDialect());
    ...
    public void testInList() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').in(true, false)).show(new GenericDialect());
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.name LIKE T0.nick ESCAPE ?) IN(?, ?)", sql);
   }

    public void testNotInList() throws Exception {
        final String sql = person.id.where(person.name.like(person.nick).escape('\\').notIn(true, false)).show(new GenericDialect());
