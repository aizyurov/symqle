...
public class TermTest extends SqlTestCase {
    ...
    public void testCast() throws Exception {
        final String sql = person.id.mult(two).cast("NUMBER(10,2)").show();
    ...
    public void testBooleanValue() throws Exception {
        String sql = person.id.where(person.id.mult(two).asPredicate()).show();
        assertSimilar("SELECT T0.id AS C0 FROM person AS T0 WHERE(T0.id * ?)", sql);
    }
    ...
    private static abstract class Scenario {
        ...
        protected abstract void runQuery(final DatabaseGate gate, final AbstractTerm<Number> term) throws SQLException;
    }
