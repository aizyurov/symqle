...
public class WhenClauseBaseListTest extends SqlTestCase {
    ...
    public void testBooleanValue() throws Exception {
        String sql = person.id.where(person.age.gt(20L).then(person.id).orWhen(person.age.gt(1L).then(person.age)).asPredicate()).show();
    ...
    public void testCollate() throws Exception {
        final String sql = person.age.gt(20L).then(person.name).orWhen(person.age.gt(1L).then(person.nick)).collate("latin1_general_ci").show();
